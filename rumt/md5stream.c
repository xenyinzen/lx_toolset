/***************************************************************************
 * Part of Running Unix Memory Test
 * Nicolas George - 2003
 * md5stream.c: generate algorithmic pseudo-random data
 ***************************************************************************/

/*
 * This file was originally written by
 * David Madore <david.madore@ens.fr>
 * as a standalone pseudo-random stream generator.
 *
 * Anything in this file is in Public Domain.
 */
...
#ifdef ORIGINAL_DAVID_MADORE_MAIN
...
main (int argc, char *argv[])
{
  ...
  if ( argc >= 2 )
    {
      for ( i=0 ; argv[1][i] ; i++ )
	MD5_feed (&context, argv[1][i]);
    }
  MD5_stop (&context, junk);
  if ( argc >= 3 )
    {
      sscanf (argv[2], "%ld", &howmuch);
      forever = 0;
    }
  else
    {
  ...
  for ( i=0 ; i<16 ; i++ )
    in[i] = 0;
  while ( forever || howmuch > 0 )
    {
      if ( forever || howmuch >= 16 )
	{
	  for ( i=0 ; i<4 ; i++ )
	    {
	      putchar (context.buf[i]&0xff);
	      putchar ((context.buf[i]>>8)&0xff);
	      putchar ((context.buf[i]>>16)&0xff);
	      putchar ((context.buf[i]>>24)&0xff);
	    }
	  howmuch -= 16;
	  MD5_transform (context.buf, in);
	  i = 0;
	  while ( ( i < 16 ) && ( ( ++in[i] & 0xffffffffUL ) == 0 ) )
	    {
	      in[i] &= 0xffffffffUL;
	      i++;
	    }
...
#else /* ORIGINAL_DAVID_MADORE_MAIN */

#include "rumt.h"

/* This code is not reentrant. It would be easy, but it is not necessary
 * here.*/

static struct md5_ctx context;
static char stopped = 0;
static unsigned long in[16];
...
void
md5stream_seed(const unsigned char *data, int length)
{
    while(length-- > 0)
	MD5_feed(&context, *(data++));
}

void
md5stream_fill(unsigned char *area, int length)
{
    int i;
    ...
    while(length >= 16) {
	for(i = 0; i < 4; i++) {
	    *(area++) = context.buf[i] & 0xff;
	    *(area++) = (context.buf[i] >> 8) & 0xff;
	    *(area++) = (context.buf[i] >> 16) & 0xff;
	    *(area++) = (context.buf[i] >> 24) & 0xff;
	}
	length -= 16;
	MD5_transform (context.buf, in);
	
	for(i = 0; i < 16 && (++in[i] & 0xFFFFFFFFUL) == 0; i++)
	    in[i] &= 0xffffffffUL;
    }
}

#endif /* ORIGINAL_DAVID_MADORE_MAIN */
